...
const Results: React.FC = () => {
  ...
  return (
    <div className="md:px-32 p-5 md:mb-20  w-full md:pt-32 pt-[84px] flex flex-col gap-4 jakartha">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 md:gap-10 gap-4">
        {accommodations.map((item) => (
          <div key={item.id} className="group shadow-xl cursor-pointer md:mt-20 mt-5 mb-8 rounded-3xl relative flex items-center justify-center">
            <Link href={`/stay/${item.id}`} className="flex flex-col justify-center items-center">
              ...
              <Image className="w-full" src={item.banner} alt={item.title} width={500} height={200} />
      ...
      <div className="flex items-center justify-center gap-10 mt-20">
        <button onClick={handlePrev} disabled={currentPage === 1} className={`${currentPage === 1 ? "opacity-50 cursor-not-allowed" : ""}`}>
          <Image src={previous} className="md:w-8 md:h-full w-[17.3px] h-[17.3px]" alt="Previous" width={32} height={32} />
        </button>
        ...
        <div className="flex items-center gap-3">
          {Array.from({ length: totalPages }).map((_, index) => (
            <span
              ...
              className={`md:w-3 md:h-3 w-[6.50px] h-[6.50px] rounded-full cursor-pointer ${
                currentPage === index + 1 ? "bg-black" : "bg-[#EAEAEA] hover:bg-gray-500"
              }`}
            />
          ))}
        </div>

        <button onClick={handleNext} disabled={currentPage === totalPages} className={`${currentPage === totalPages ? "opacity-50 cursor-not-allowed" : ""}`}>
          <Image src={next} className="md:w-8 md:h-full w-[17.3px] h-[17.3px]" alt="Next" width={32} height={32} />
        </button>
      </div>
    </div>
  );
